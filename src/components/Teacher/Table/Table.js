...
import { deleteTeacher } from "../../../Middlewares/Teacher/DeleteTeacher";
...
const Table = ({ records }) => {
  ...
  const students = records.map((teacher) => {
    return (
      <tr className="text-center text-xl font-pageFont" key={teacher.id}>
        <td className="capitalize py-3">{teacher.name}</td>
        ...
        <td className="flex justify-center gap-3 py-3 cursor-pointer">
          <svg
            ...
            onClick={() => {
              dispatch(getTeacher(teacher.id)).then((res) => {
                if (res.meta.requestStatus === "fulfilled") {
                  navigate(`${teacher.id}/details`);
                }
              });
            }}
          >
